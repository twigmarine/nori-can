const {
  at, cond, curry, flow, get, isEmpty, isMap, isPlainObject, isString,
...
const processPackets = curry((getPgnInfo, state, packets) => {
  if (isEmpty(packets)) return []
  if (isPlainObject(packets)) return [parse(getPgnInfo, state, packets)]
...
const processData = curry((getPgnInfo, info) => addFields(parse(getPgnInfo, new Map(), info)))
