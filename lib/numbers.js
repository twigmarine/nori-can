...
function fromNumber(num, byteLength, signed) {
  ...
  if (!numInfo) {
    console.error('no number info found for size', byteLength)
    return undefined
  }
  const funcName = signed ? 'int' : 'uint'
  const func = numInfo[funcName].fromNum
  if (!func) {
    console.error(`No fromNum ${funcName} func found for ${byteLength} bytes.`)
    return undefined
  }
  return func(num)
}
