const {
  identity, isNull, isNumber, isUndefined, noop,
} = require('lodash/fp')
...
const toBasicNum = (methodName) => (view, littleEndian) => view[methodName](0, littleEndian)

function fromBasicNum(methodName) {
  return (view, offset, num, littleEndian = true) => {
    if (!littleEndian) {
      // console.log(methodName, offset, num, littleEndian)
      view.bigEndian = true // eslint-disable-line no-param-reassign
    }
    // Always round when creating a basic number.
    view[methodName](offset, Math.round(num), littleEndian)
    return view.buffer
  }
}
...
function fromNumber(view, num, fieldInfo) {
  const {
    bitOffset, bitLength, byteLength, littleEndian, signed,
  } = fieldInfo
  // Variable number of reserved bits, all set to logic "1".
  if (isReserved(fieldInfo) || isNull(num) || isUndefined(num)) {
    // Assume maxing out the field is the same as null.
    return view.setBits(bitOffset, getMaxUint(bitLength), bitLength)
  }
  if (!byteLength) { // Bits
...
function toNumber(view, signed, littleEndian = true) {
  ...
  const numVal = numInfo.toNum(view, littleEndian)
  return numVal === numInfo.maxVal ? null : numVal
}

// Maybe this should get the view and the entire field object?
function toPrecise(view, {
  signed, resolution, littleEndian, offset,
}) {
  // 1e-16 === Math.pow(10, -16) max 15 decimal places
  let num = toNumber(view, signed, littleEndian)
