const {
  isTypedArray, map, padCharsStart, parseInt, sumBy, trimChars,
} = require('lodash/fp')
...
const hexToBuff = (x) => (x[2] === x[5] ? hexSpStrBuff(x) : hexStrBuff(x))
...
const arrayBufferEqual = (buff1, buff2) => dataViewEqual(
  new DataView(buff1.buffer || buff1),
  new DataView(buff2.buffer || buff2),
)
module.exports = {
  arrayBufferEqual,
  arrBuff,
