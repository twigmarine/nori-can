const {
  encodeCanId, parseCanId, parseEncode, pgnAddressable,
} = require('./canId')

/* globals describe test expect */
/* eslint no-bitwise: 0 */

describe('pgnAddressable', () => {
  test('Return true if destination is addressable.', () => {
    expect(pgnAddressable(0xE800)).toBe(true)
    expect(pgnAddressable(0xEA00)).toBe(true)
    expect(pgnAddressable(0xEB00)).toBe(true)
    expect(pgnAddressable(0xEC00)).toBe(true)
    expect(pgnAddressable(0xEF00)).toBe(true)
    expect(pgnAddressable(0x1EF00)).toBe(true)
    expect(pgnAddressable(0x1FE17)).toBe(false)
    expect(pgnAddressable(65280)).toBe(false) // FF00
    expect(pgnAddressable(126983)).toBe(false)
    expect(pgnAddressable(126998)).toBe(false)
  })
})
...
describe('parseCanId', () => {
  test('Return object with canId broken into properties', () => {
    ...
    expect(parseCanId(0x1806E5F4)).toEqual({
      canId: 0x1806E5F4, dst: 229, src: 244, pgn: 1536, prio: 6,
    })
    expect(parseCanId(0x18FF50E5)).toEqual({
      canId: 0x18FF50E5, dst: 255, src: 229, pgn: 65360, prio: 6,
    })
  })
})
