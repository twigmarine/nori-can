...
function encodeLawicel({ data, ...canIdInfo }) {
  const canId = encodeCanIdString(canIdInfo)
  return `T${canId}${data.byteLength}${byteString(data, '')}\r`
}
...
function parseString(line, formatId) {
  ...
  const parser = get('parser', parsersIndex[formatId] || findStyle(n2kStr))
  if (!parser) return noParserErr(line + (formatId ? ` - ${formatId}` : ''))
  return parser(n2kStr)
}
exports.parseString = parseString

// Process a string that may have many lines/messages/frames.
function processStringInput(input, formatId) {
  // Is running compact() here too computationally expensive?
  const messages = compact(splitLines(input))
  if (!messages.length) return undefined
  return messages.map((x) => parseString(x, formatId))
}
