const {
  at, conforms, curry, defaults, eq,
  filter, flow, get, groupBy, has, inRange, isMatch, isPlainObject,
  keyBy, map, multiply, overEvery, overSome, propertyOf,
  some, sumBy, toNumber, update, zipObject,
} = require('lodash/fp')
...
const prepPgn = update('fields', map(prepField))
...
function prepInfo(pgnInfoOptions) {
  if (pgnInfoOptions.length === 1) return pgnInfoOptions[0]
