const {
  ...
  set, some, startsWith, sumBy, toNumber, update, upperFirst, values,
} = require('lodash/fp')
...
const {
  addField, doProp, move, propDo, replaceField, setField, setWith,
} = require('prairie')
const humps = require('lodash-humps')
const { pgnAddressable } = require('./canId')
...
const fieldDataFixes = (position) => flow(
  onTrue(has('byteLength'), setWith('bitLength', 'byteLength', multiply(8))),
  onTrue(isByteSize, setWith('byteLength', 'bitLength', divideBy(8))),
  checkText,
...
const isProprietaryMultiAddressable = eq(0x1EF00)
const isProprietaryMultiGlobal = inRange(0x1FF00, 0x1FFFF + 1)
const isProprietaryMulti = overSome([
  isProprietaryMultiAddressable, // Destination Addressable
  isProprietaryMultiGlobal, // Global
])

// Single Frame Proprietary
// Destination Addressable - 061184
const isProprietarySingleAddressable = eq(0xEF00)
// Destination Global - 065280 through 065535
const isProprietarySingleGlobal = inRange(0xFF00, 0xFFFF + 1)
// Any Single Frame Proprietary
const isProprietarySingle = overSome([
  isProprietarySingleAddressable,
  isProprietarySingleGlobal,
])
...
const isReserved = propDo('id', startsWith('reserved'))
...
const fixPgn = flow(
  ...
  setWith('addressable', 'pgn', pgnAddressable),
  addPriority,
...
const prepFields = update('fields', flow(fixFields, map(prepField)))
...
function createGetInfo(pgns) {
  const getInfo = createPgnMap(pgns)
  const byId = pgnInfoById(isPlainObject(pgns) ? values(pgns) : pgns)
  const infoById = propertyOf(byId)
  return ({ field, id, pgn }) => (id ? infoById(id) : getInfo(pgn, field))
}
