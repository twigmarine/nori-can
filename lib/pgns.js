const {
  at, conforms, curry, defaults, eq, filter, flow, get, groupBy, has, inRange,
  isPlainObject, map, multiply, overEvery, overSome, propertyOf,
  some, sumBy, toNumber, update,
} = require('lodash/fp')
const {
...
const { industryCodes } = require('./codes')
...
const findPgnInfo = curry((pgnMap, pgn) => {
  ...
  if (!info) return info.proprietary ? getProprietary({ pgn }) : { pgn }
  // Will need to process match later.
  if (info[0].matchFields) return info
  if (info.length !== 1) {
    console.error('many pgn options but no match fields', info)
  }
  return info[0]
})
...
const prepPgn = flow(
  map(update('fields', map(prepField))),
  (x) => [x[0].pgn, x],
  // (x) => {
  //   console.log(x[0])
  //   return x
...
const createPgnMap = flow(map(prepPgn), newMap)
