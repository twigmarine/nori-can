const { flow, isEmpty, pick } = require('lodash/fp')
const { mergeFieldsWith, setField, setWith } = require('prairie')
...
const fieldTypes = {
  ...
  // 'deviceClass': setWith('label', 'value', getIndustryName)
  // deviceFunction
}
const devClassInfo = flow(
  getClass,
  pick(['label', 'labelDetails', 'deprecated']),
)
const fieldIds = {
  deviceClass: mergeFieldsWith('value', devClassInfo),
}
const processTypes = (x) => {
  if (fieldTypes[x.type]) return fieldTypes[x.type](x)
  if (fieldIds[x.id]) return fieldIds[x.id](x)
  return x
}
