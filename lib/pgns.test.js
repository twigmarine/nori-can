...
describe('createGetInfo', () => {
  ...
  test('Return info by id.', () => {
    const info = getInfo({ id: 'isoRequest' })
    expect(info)
      .toMatchObject({ pgn: 59904 })
  })
})
describe('field2Arr', () => {
...
describe('getProprietary', () => {
  test('Test unknown pgn', () => {
    ...
    expect(res.fieldIndex.industryCode.bitLength).toBe(3)
    expect(res.fieldIndex.manufacturerCode.name).toBe('Manufacturer Code')
    delete res.fieldIndex
    expect(res)
      .toEqual({
        pgn: 65280,
        pgnHex: 'FF00',
