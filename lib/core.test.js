...
const { canboatGetInfo, createGetInfo, processStringInput } = require('.')
...
describe('parser', () => {
  test('process many line input string', () => {
    ...
    const info = getPgnInfo({ pgn: 129039 })
    info.byteLength = 27
    info.fields.push({
      bitLength: 15,
      bitOffset: 201,
    ...
    expect(res[0]).toEqual({
      ...
      field: {
        ...
        regionalApplication: {
          bitLength: 8, name: 'Regional Application', value: 0,
        },
        regionalApplication1: {
          bitLength: 2, name: 'Regional Application', value: 0,
        },
        unitType: {
          bitLength: 1,
          type: 'Lookup table',
...
describe('processPackets', () => {
  const msgA = '04:12:22.228 R 15FD0A05 FF 00 01 F3 2A 01 00 FF'
  const msgB = '04:12:22.229 R 15FD0A05 FF 01 01 91 17 0A 00 FF'
  const packets = processStringInput(`${msgA}\n${msgB}`)
  const state = new Map()
  test('single packet', () => {
    const res = processPackets(getPgnInfo2, state, packets.slice(0, 1))
    expect(res.length).toBe(1)
    const res2 = processPackets(getPgnInfo2, state, packets.slice(1, 2))
    expect(res2.length).toBe(1)
    expect(res2[0].input).toBe(msgB)
  })
  test('two pressure packets', () => {
    const res = processPackets(getPgnInfo2, state, packets)
