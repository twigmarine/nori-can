...
const { createPgnStrings, encode, getFieldValue } = require('./encodePgn')
...
describe('getFieldValue', () => {
  ...
  test('field direct value', () => {
    expect(getFieldValue(input1, fieldInfo)).toBe(100)
  })
  test('field value prop', () => {
    expect(getFieldValue(input2, fieldInfo)).toBe(102)
  })
  test('match prop wins', () => {
    const fieldInfo2 = { ...fieldInfo, match: 30 }
    expect(getFieldValue(input1, fieldInfo2)).toBe(30)
    expect(getFieldValue(input2, fieldInfo2)).toBe(30)
  })
  test('use default value', () => {
    expect(getFieldValue(input1, { id: 'foo', default: 10 })).toBe(10)
    expect(getFieldValue(input2, { id: 'bar', default: 20 })).toBe(20)
  })
})
...
describe('encode', () => {
  ...
  test('test temperature', () => {
    const res = encode(getPgnInfo, temperaturePacket)
...
describe('createPgnStrings', () => {
  const expectedString = '19FD0C00 FF 00 08 EE 80 04 FF FF\r\n'
  test('create pgn strying', () => {
    const strs = createPgnStrings(getPgnInfo, 'YDRAW', temperaturePacket)
    expect(strs[0]).toBe(expectedString)
  })
})
