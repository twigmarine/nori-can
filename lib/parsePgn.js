...
const {
  dataPosition, fieldDataFixes, getByteLength, isReserved, startLengthByte,
} = require('./pgns')
...
const getFieldData = curry((data, field) => {
  ...
  if (field.byteEnd) {
    ...
    if (field.type === 'Binary data' && !isReserved(field)) {
      return data.slice(field.byteStart, field.byteEnd)
    }
    return toPrecise(
...
function addDataFields(input, info, fieldData, fields) {
  ...
  if (info.fields === fields || !fields) {
    return { ...input, info, fieldData }
  }
  return {
    ...input,
    info: { ...info, byteLength: getByteLength(fields), fields },
    fieldData,
  }
}
