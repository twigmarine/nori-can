...
const getFieldsData = (input, fields) => map(getFieldData(input.data), fields)
...
function addInfoFieldData(input, info) {
  ...
  function tryOption(infoOption) {
    ...
    const matched = infoOption.matchFields.every(({ position, match }) => (
      getFieldData(input.data, infoOption.fields[position]) === match))
    if (matched) packetInfo = splitPgn(input, infoOption)
    return matched
  }
