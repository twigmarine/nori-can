...
const fixPgn2 = flow(fixPgn, humps)

// organize PGNs
const fixPgns = flow(
  get('PGNs'),
  map(fixPgn),
  groupBy('PGN'), // There are sometimes more than one info set per pgn.
)
const pgnMap = flow(
  get('PGNs'),
  map(fixPgn2),
...
const organizedPGNs = fixPgns(pgns)
...
// console.log(values(organizedPGNs).map(first).slice(0, 5))
// console.log(filter(isSingleRepeat, values(organizedPGNs).map(first)))
const getPgn = propertyOf(organizedPGNs)
module.exports = {
  basicGetPgnInfo,
  field2Arr,
  fixField,
