const { curry, flow, isNumber, map, set } = require('lodash/fp')
...
const { byteString, dataViewEqual, hexStrBuff } = require('./utilities')
...
function parser() {
  let state = new Map()
  return input => {
    return parse(basicGetPgnInfo, state, input)
  }
}
